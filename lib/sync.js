...
function Sync(config) {
	...
	return self.ensureBucketExists().then(function () {
		...
		return self.ensureDistributionDefaultRootObj();
	}).then(function () {
		var uploadedFiles = [];
		...
		return BPromise.map(filePaths, function (filePath) {
			started++;
			...
			let s3Params = Object.assign({}, params.s3Params, {
				...
				Key: `${params.s3Params.Prefix}/${key}`
			});
		...
		}, {concurrency: self.config.s3Options.concurrency}).then(function(data) {
			if (Sync.isModule) {
